...
class VedicRAG:
    ...
    def _template_answer(self, question: str,
                         verses: list[SourceVerse]) -> str:
        """Scholarly cited synthesis when external LLM pipeline is not in use."""
        if not verses:
            return "No relevant verses found for this inquiry in the Vedic corpus."

        from database.supabase_client import SupabaseDB
        db = SupabaseDB()

        top = verses[0]
        verse_details = db.get_verse(top.verse_id)

        translation = verse_details.get("translation_en", "") if verse_details else ""
        devanagari = verse_details.get("devanagari", "") if verse_details else top.text
        iast = verse_details.get("iast", "") if verse_details else ""

        # Check for science link
        science_links = db.get_science_links(verse_id=top.verse_id)

        lines = [
            f"ANSWER:",
            f"According to the classical Sanskrit canon, this topic is directly addressed in {top.verse_id}.",
            f"",
            f"CITED SHLOKA ({top.verse_id}):",
            f"{devanagari}",
            f"",
            f"TRANSLITERATION (IAST):",
            f"{iast}",
            f"",
            f"MEANING & PHILOSOPHICAL EXPOSITION:",
            f"\"{translation}\"",
        ]
        ...
        if science_links:
            ...
            lines.extend([
                f"",
                f"MODERN SCIENCE PARALLEL ({s.get('domain', 'Science')}):",
                f"• Reference: {s.get('modern_title', '')} ({s.get('modern_ref', '')})",
                f"• Correlation: {s.get('description', '')}"
            ])

        if len(verses) > 1:
            lines.append("")
            lines.append("ADDITIONAL RELATED PASSAGES:")
            for v in verses[1:4]:
                other = db.get_verse(v.verse_id)
                tr = f" — \"{other.get('translation_en', '')[:90]}...\"" if other and other.get("translation_en") else ""
                lines.append(f"  • {v.verse_id}{tr}")
