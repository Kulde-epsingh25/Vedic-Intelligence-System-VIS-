...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "vis-verses")
...
class Embedder:
    ...
    def _init_pinecone(self) -> None:
        if PINECONE_API_KEY and "pcsk_" in PINECONE_API_KEY:
            try:
                from pinecone import Pinecone
                pc = Pinecone(api_key=PINECONE_API_KEY)
                self.pinecone_index = pc.Index(PINECONE_INDEX_NAME)
                logger.success(f"Pinecone connected to index '{PINECONE_INDEX_NAME}'")
    ...
    def semantic_search(self, query: str, n_results: int = 10,
        ...
        """
        # 1. Primary: Pinecone Cloud Vector Search
        if self.pinecone_index is not None and self.model is not None:
            try:
                query_embedding = self.embed_text(query)
                filter_dict = {"source_text_id": {"$eq": filter_text}} if filter_text else None
                ...
                if res and res.matches:
                    ...
                    for i, m in enumerate(res.matches):
                        meta = m.metadata or {}
                        text_repr = f"{meta.get('devanagari', '')}\n{meta.get('iast', '')}\n{meta.get('translation_en', '')}"
                        output.append({
                            "rank": i + 1,
                            "verse_id": m.id,
                ...
                logger.warning(f"Pinecone search failed ({e}). Falling back to Chroma/local.")

        # 2. Secondary: ChromaDB Local Vector Search
        if self.collection is not None and self.model is not None:
            try:
                ...
                logger.warning(f"ChromaDB search failed ({e}). Falling back to local keyword matching.")

        # Local database intelligent fallback
        from database.supabase_client import SupabaseDB
        db = SupabaseDB()
        verses = db.search_verses(source_text_id=filter_text, limit=100)
        query_words = set(query.lower().split())

        scored = []
        for v in verses:
            score = 0
            text_blob = f"{v.get('iast', '')} {v.get('translation_en', '')} {v.get('devanagari', '')}".lower()
            for qw in query_words:
                if qw in text_blob:
                    score += 1
            if score > 0 or len(query_words) == 0:
                similarity = min(0.98, round(0.5 + (score * 0.15), 4))
                scored.append((similarity, {
                    "rank": 0,
                    "verse_id": v["verse_id"],
                    "text": f"{v['devanagari']}\n{v['iast']}\n{v.get('translation_en', '')}",
                    "similarity": similarity,
                    "metadata": {"source_text_id": v.get("source_text_id", ""), "era": v.get("era", "")}
                }))
