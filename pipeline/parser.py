    ...
    def _load_vidyut(self) -> None:
        ...
        try:
            ...
            try:
                ...
                data_dir = Path(os.getenv("MODELS_DIR", "./models")) / "vidyut_data"
                if not data_dir.exists():
                    import vidyut
                    if hasattr(vidyut, "download_data"):
                        data_dir.mkdir(parents=True, exist_ok=True)
                        vidyut.download_data(str(data_dir))
                if data_dir.exists():
                    self.kosha = Kosha(str(data_dir))
            if self.kosha:
                logger.success("vidyut Kosha loaded successfully")
        except Exception as e:
            logger.info(f"vidyut Kosha optional download deferred ({e}); using tokenizer fallback.")
